	...
	public async run(ctx: Context): Promise<any> {
		...
		if (subcommand === "setup") {
			...
			try {
				...
				console.error("[VoiceMaster Setup Error]", e);
				return ctx.sendMessage({
					content: "Failed to setup VoiceMaster. Make sure I have `Manage Channels` permission.",
				});
			}
		}
		...
		if (subcommand === "reset") {
			...
			if (!voiceMaster) {
				return ctx.sendMessage({
					components: [buildPanel("VoiceMaster", "There is no VoiceMaster setup in this server.")],
					flags: MessageFlags.IsComponentsV2,
				});
			}
			...
			return ctx.sendMessage({
				components: [buildPanel("VoiceMaster Reset", "VoiceMaster has been reset.")],
				flags: MessageFlags.IsComponentsV2,
			});
		}
	}
