...
export default class LockAll extends Command {
	...
	public async run(ctx: Context): Promise<any> {
		...
		try {
			...
			if (channels.size === 0) {
				const embed = new EmbedBuilder().setColor(Colors.Yellow).setDescription("No channels found to lock");
			...
			const embed = new EmbedBuilder().setColor(Colors.Red).setDescription("<:Cross:1375519752746958858> An error occurred during mass lock");
