...
export default class MuteMenu extends Menu {
	constructor(client: BaseClient) {
		super(client, { id: "voice-mute-menu" });
	}
	...
	public async execute(interaction: UserSelectMenuInteraction): Promise<any> {
		...
		try {
			await voice.permissionOverwrites.edit(member.id, { Speak: false }, {
				reason: `VoiceMaster room mute by ${interaction.user.tag} (${interaction.user.id})`,
			});
			return reply("Muted the selected member in this room.");
		} catch (error) {
			console.error("[VoiceMaster Mute Error]", error);
			return reply("Failed to mute the selected member.");
		}
	}
}
