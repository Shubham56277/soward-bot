...
export default class Disconnect extends Button {
	...
	public async execute(interaction: ButtonInteraction): Promise<any> {
		...
		if (voice instanceof VoiceChannel) {
			...
			const menu = new UserSelectMenuBuilder().setCustomId("voice-disconnect-menu").setPlaceholder("Select user to disconnect").setMinValues(1).setMaxValues(1)
